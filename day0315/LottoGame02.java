...
public class LottoGame02 {
    ...
    public static void main(String[] args) {
        ...
        // 2. 사용자로부터 몇게임 할지 입력받아서
        // 총 게임 수와 그리고 해당 게임에 자동 vs 수동 결정하기
        // A. 총 게임 수 입력 받기
        int gameNumber = ScannerUtil.nextInt(sc, "총 게임 수: ");
        // B. 2차원 배열 선언
        ...
        for (int i = 0; i < userNumbers.length; i++) {
            ...
            int userChoice = ScannerUtil.nextInt(sc, "1. 자동 2. 수동", 1, 2);
            if (userChoice == 1) {
                userNumbers[i] = addRandomNumber(userNumbers[i]);
            } else if (userChoice == 2) {
                userNumbers[i] = insert(userNumbers[i], sc);
            }
            // D. 정렬
            sort(userNumbers[i]);
        }
        ...
        for (int i = 0; i < userNumbers.length; i++) {
            ...
            System.out.print(", 맞은 갯수: " + count + "개, ");
            if (count >= 2) {
                System.out.print("등수: " + (6 - count + 1) + "등");
            } else {
    ...
    // 2개의 배열을 비교해서 같은 숫자가 몇 개인지 찾아주는 메소드
    ...
    // 수동으로 숫자를 넣는 메소드
    public static int[] insert(int[] numbers, Scanner sc) {
        while (ArrayUtil.size(numbers) < SIZE) {
            System.out.println((ArrayUtil.size(numbers) + 1) + "번 숫자");
            int number = ScannerUtil.nextInt(sc, "숫자를 입력해주세요.", MIN, MAX);
