...
// 2. Set
//    셋은 중복 입력이 불가능하고 동적 할당이 구현된 배열이라고 생각하면 편하다.
// 3. Map
...
public class Ex01ArrayList {
    public static void main(String[] args) {
        // 그간 우리가 ArrayUtil.java에 구현해놧던 모든 메소드들이
        // 모두 ArrayList가 독자적으로 가지고 있는 메소드들이다.

        // ArrayList 선언하기
        // ArrayList를 선언하는 방법은 다음과 같다.
        // ArrayList<클래스> 이름 = new ArrayList<>(); // 이름 = 리스트명
        // <> 는 자바에서 템플릿이라고 부른다.
        ...
        // 따라서, 여러분들이 숙제로 해올
        // 로또게임의 ArrayList 버젼 같은 경우, 저 <>에 Integer 라고 적어주면 된다!

        // Student 클래스의 ArrayList를 만들어 보자.
        ArrayList<Student> list = new ArrayList<>();

        // 1. 현재 ArrayList 객체의 크기를 알고 싶을 땐
        // size()메소드를 실행하면 된다.
        ...
        // 3. 리스트에 존재한 객체를 꺼내올 때에는
        //    get(index) 메소드를 실행하면 된다.
        //    당연히 index는 0부터 크기 -1 까지이다.
        System.out.println("3. get(index)");
        System.out.println("list.get(0).showuInfo(): ");
        list.get(0).showInfo(); // 2번에서 s1을 넣었음
        System.out.println();
        
        // 4. 리스트에 특정 index에 새로운 객체를 추가할 때에는
        ...
        list.add(0, s3); // 0번에 s3 들어감
        System.out.println("list.get(0).getName(): "+list.get(0).getName());
        System.out.println();
        
        // 5. 리스트에 특정 index에 있는 객체를 제거할 때에는
        ...
        list.remove(1); // 1번 인덱스 삭제(s1 삭제)
        System.out.println("list.get(1).getName(): "+list.get(1).getName());
        System.out.println();
        
        // 6. 리스트의 특정 index에 있는 객체를 바꿀 때에는
        //    set(index, e)를 실행하면 된다.
        //    이 메소드의 결과값은 void 가 아니라
        //    해당 index에 원래 있던 객체가 결과값이 된다!
        System.out.println("6. set(index, e)");
        System.out.println("list.get(0).getName(): "+list.get(0).getName());
        Student temp = list.set(0, s1);//(temp에 s3 들어감// 0번 자리에 s1추가)
        System.out.println("list.get(0).getName(): "+list.get(0).getName());
        list.set(0, temp); // ( temp에 있던 s3가 들어옴 // 0번 자리의 s1은 리스트에서 빠짐)
        System.out.println("list.get(0).getName(): "+list.get(0).getName());
        System.out.println("list.get(1).getName(): "+list.get(1).getName());
        System.out.println();
        ...
        // 7. 리스트안에 특정 객체가 존재하는지 확인하고 싶을 때에는
        //    contains(e) 메소드를 실행하면 된다.
        //    이때에는 완전히 동일한 객체 뿐만 아니라
        ...
        System.out.println("list.contains(s1): "+list.contains(s1)); // 리스트에서 s1은 빠진상태
        System.out.println("B. equals()가 true가 나오는 객체를 사용하는 경우");
        System.out.println("list.contains(s22): "+list.contains(s22)); // 객체의 이름은 다르지만 필드값이 같아서 equals 가 나온다.
        System.out.println("list.contains(s11): "+list.contains(s11));
        System.out.println();
        
        // 8. 리스트 안에 특정 객체의 가장 먼저 등장하는 index가 궁금할 때에는
        ...
        System.out.println("B. equals()가 true가 나오는 객체를 사용하는 경우");
        System.out.println("list.indexOf(s22): "+list.indexOf(s33));
        ...
        //     다른 객체 이지만 equals(true)가 나오는 객체는 삭제가 된다.
