...
public class StudentViewer {
    ...
    public void register() {
        ...
        s.setPassword(ScannerUtil.nextLine(sc, message));
        
        // 1-6. 사용자의 이름을 입력받아서 학생 객체에 추가해준다.
        message = new String("이름을 입력해주세요.");
        s.setName(ScannerUtil.nextLine(sc, message));
        
        // 1-7. 사용자의 소속 반을 입력받아서 학생 객체에 추가해준다.
        message = new String("소속 반을 입력해주세요.");
        s.setGroup(ScannerUtil.nextInt(sc, message));
        
        // 1-8. 정보가 들어가있는 학생객체를 컨트롤러에 add한다.
        controller.add(s);
    }
    ...
    public boolean logIn() {
        ...
        // 2-3. 사용자로부터 아이디를 입력받는다.
        message = new String("아이디를 입력해주세요.");
        s.setUsername(ScannerUtil.nextLine(sc, message));
    ...
    //    int type의 경우 VALUE_ALL 이면 모든 학생 보여주기 이고, VALUE_NO_INPUT 이면 아직 성적입력이 안된 학생으로 생각하면
    //    된다.
    public void printList(int type ,int group) {
       
    while(true) {
        // 4-1. 출력할 목록을 컨트롤러를 통해서 불러온다.
        ...
        if(list.isEmpty()) {
            System.out.println("입력할 학생이 존재하지 않습니다.");
            break;
        }
        ...
        for(StudentDTO s : list) {
            // 성적 입력 여부를 X/O로 보여줄 String 객체 input
            String input;
            if(s.isInputSwitch()) {
                input = new String("O");
            } else {
        ...
        // 4-2. 개별보기할 번호를 입력 받는다.
        String message = new String("개별보기할 번호를 입력하거나 뒤로 가시려면 0을 입력해주세요.");
        int id = ScannerUtil.nextInt(sc, message);
        
        // 4-3. 입력한 번호로 controller의 selectOne()을 실행한다.
        StudentDTO s = controller.selectOne(id);
        
        // 4-4. 번호가 유효한 번호인지를 확인하고 유효하지 않으면 다시 입력을 받는다.
        ...
        // 4-5. 만약 개별보기를 선택했으면 해당 메소드를 호출해주고
        //      아니면 break 한다.
        if(id == 0) {
            break;
        } else {
    ...
    public void printOne(int id) {
        ...
        // 1~2까지 입력을 받고, 그 외에는 1만 입력을 받게 한다.
        
        // 사용자의 입력값을 저장할 int userChoice
        int userChoice;
        
        if(message.contains("2")) {
            userChoice = ScannerUtil.nextInt(sc, message, 1, 2);
            if(userChoice == 1) {
                // 수정 메소드 실행
                showUpdate(id);
                printOne(id);
            ...
            userChoice = ScannerUtil.nextInt(sc, message, 1, 1);
        }
    ...
    // 뷰어의 필드인 컨트롤러에서 해당 학생의 이름을 return해주는 메소드
