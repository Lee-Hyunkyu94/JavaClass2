...
public class GradeBooK01 {
    ...
    private static void printAll() {
        ...
        while (true) {
            ...
            int userChoice = ScannerUtil.nextInt(sc, message, 0, list.size()) - 1; // 사용자의 입력값에 -1 을 하는거임.

            // 사용자가 0을 입력하면 -1이 되서 userChoice에 저장되므로
            // userChoice 가 -1이면 break를 통해 목록을 보는 것을 종료시킨다.
            ...
            if (userChoice == -1) {
                ...
                // userChoice 를 파라미터로 하는 개별보기 메소드
    ...
    private static void delete(int index) {
        // 먼저 사용자로부터 정말로 삭제할 지를 물어보고
        // 만약 사용자가 y 혹은 Y 외에 다른 값을 입력하면 삭제를 하지 않는다.
        // 사용자가 정확하게 대소문자 y를 입력할 때에만 삭제하고
        // 삭제되면 목록으로 돌아가게 한다.
        // 삭제되지 않았을 때에는 여기서 printOne(index)를 실행시켜 준다.
