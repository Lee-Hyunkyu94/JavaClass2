    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            if (userChoice == 1) {
                ...
                if (idx < STUDENT_SIZE) { // idx 는 0부터 시작이므로 0 ~ STUDENT_SIZE -1 이다.
                    // 아직 5명 미만이 입력됬으므로
                    // 입력 가능
                    ...
                    // 이름 입력
                    sc.nextLine(); // 위에는 int였다가 지금은 String!!!
                ...
                if (idx > 0) {
                    // idx 가 0보다 크다라는것은
                    // 현재 입력한 index가 0보다 크다.
                    // 즉 1명 이상이 입력되어있다라는 뜻이 된다.

                    // 그럴 경우 0번 idx부터 idx 보다 작을때가지 i for 문을 사용하여
                    // 내용을 출력해주면 된다.
                    for (int i = 0; i < idx; i++) {
                        System.out.println();
                    ...
                    System.out.println();
                    System.out.println("아직입력된 학생이 존재하지 않습니다.");
                    System.out.println();
