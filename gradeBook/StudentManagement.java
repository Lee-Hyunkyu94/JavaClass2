...
요구조건:
   ...
   2. 로그인 후에 메인화면에서 학생목록 보기와 교사 정보 보기가 있다. // 로그인한 선생/ 학생 각각 하나씩?
   3. 학생은 자신의 성적과 다른 학생의 성적을 볼 수만 있다. // 성적 저장한 int 변수
   4. 학생은 교사에 대한 평가를 남기거나 자기가 남긴 평가를 삭제할 수 있다. // 평가 저장할 int 변수
...
public class StudentManagement01 {
    ...
    // 회원 가입시 학생의 id값을 자동으로 처리해줄 int 변수
    private static int curStudentId = 1;
    // 회원 가입시 선생의 id값을 자동으로 처리해줄 int 변수
    private static int curTeacherId = 1;
    // 평가 등록시 평가의 id값을 자동으로 처리해줄 int 변수
    ...
    private static void logIn() {
        ...
        while (true) {
            ...
            int type = ScannerUtil.nextInt(sc, message, 0, 2);
            if (type == 0) {
                break;
            }

            message = new String("아이디: ");
            String username = ScannerUtil.nextLine(sc, message);
            ...
            // 3. 만약 logInStudent 나 logInTeacher 중 하나라도 null이 아닐 경우
            // break 한다.
            if (logInStudent != null || logInTeacher != null) {
                break;
            }
    ...
    private static void setLogIn(int type, String username, String password) {
        if (type == 1) {
            ...
            // username 과 password를 담을 Student 임시객체
            Student temp = new Student();

            // 임시 객체에 username 과 password를 넣어준다.
            temp.setUsername(username);
            temp.setPassword(password);
    ...
    private static void register() {
        ...
        int type = ScannerUtil.nextInt(sc, message, 1, 2);
        ...
        // 존재하지 않는 username 값이 들어올 때까지 다시 입력을 받는다.

        while (validateUsername(type, username)) {
            System.out.println("이미 존재하는 아이디입니다.");
            username = ScannerUtil.nextLine(sc, message);
        }
        ...
        String name = ScannerUtil.nextLine(sc, message);

        // 2-4. 소속 반 입력 받기
        message = new String("소속 반을 입력해주세요.");
        int group = ScannerUtil.nextInt(sc, message);
        ...
        if (type == 1) {
            ...
            // Teacher t 객체에 교사 번호를 넣는다.
            t.setId(curTeacherId++);

            // Teacher t 객체에 교사 아이디를 넣는다.
            t.setUsername(username);

            // Teacher t 객체에 교사 비밀번호를 넣는다.
            t.setPassword(password);

            // Teacher t 객체에 교사 이름을 넣는다.
            t.setName(name);

            // Teacher t 객체에 교사 소속 반을 넣는다.
            t.setGroup(group);
    ...
    // 2-3. 해당 종류의 리스트에 똑같은 username이 있는지 확인하는 중복 메소드
    private static boolean validateUsername(int type, String username) {
        if (type == 1) {
            // 콜렉션의 경우, 단순히 출력이나 비교를 할 때에는
            // for문을 좀 간략화된 버젼으로 쓸 수있다.
            // 이렇게 간략화된 for문을 forEach문 이라고 부른다. (ArrayList 나 collection 같이 단순 집합의 경우)
    ...
    private static void showMenu() {
        ...
        while (true) {
            String message;
            int userChoice;
            if (logInTeacher != null) { // 교사가 로그인
                message = new String("1. 전체 학생 목록 보기 2. 성적 미등록 학생 목록 보기 3. 교사 평가 보기 4. 종료");
                userChoice = ScannerUtil.nextInt(sc, message, 1, 4);
                if (userChoice == 1) {
                    // 전체 학생을 보는 메소드 실행
    ...
    private static void printAll() {
        while (true) {
            ...
            // 지금현재 소속 반의 리스트를 for 문을 이용하여 전부 출력한 후에
            // 개별보기, 뒤로 가기 를 한다.
            ...
            // 2. 사용자가 입력한 값이 유효한 지 확인한다.
            // 만약 유효하지 않으면 유효한 값을 입력할 때까지 다시 입력을 받는다.

            // 2-1. 입력한 값이 유효한 지 확인하기 위해 id가 userChoice 인 Student 객체를 만든다.
            Student s = new Student();
            s.setId(userChoice);
    ...
    private static void insertScore(int id) {
        ...
        s.setKorean(ScannerUtil.nextInt(sc, message, 0, 100));
        // 영어 점수 입력
        message = new String("영어 점수를 입력해주세요.");
        s.setEnglish(ScannerUtil.nextInt(sc, message, 0, 100));

        // 수학 점수 입력
        message = new String("수학 점수를 입력해주세요.");
        s.setMath(ScannerUtil.nextInt(sc, message, 0, 100));
